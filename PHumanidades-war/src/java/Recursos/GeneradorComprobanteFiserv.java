...
/**
 * Genera el comprobante en PDF de un pago realizado vía QR (Fiserv). Mismo
 * diseño que {@link GeneradorComprobanteMP}, pero sin depender del SDK de
 * Mercado Pago: los datos de la transacción llegan sueltos porque la
 * respuesta de Fiserv se procesa como JSON genérico (javax.json), no como un
 * objeto tipado.
 */
public class GeneradorComprobanteFiserv {

    public static ByteArrayOutputStream generarComprobante(String transactionId, String status, BigDecimal monto,
            String metodoPago, Alumno alumno, Cohorte cohorte) throws Exception {
        ...
        addDetailCell(pagoTable, "ID de Operación (Fiserv)", transactionId != null ? transactionId : "No disponible", labelFont, valueFont, lightGray, borderGray);
        addDetailCell(pagoTable, "Método de Pago", metodoPago != null ? metodoPago.toUpperCase() : "QR", labelFont, valueFont, lightGray, borderGray);
        ...
        String cleanStatus = status != null ? status.toUpperCase() : "APROBADO";
        ...
        String rawAmount = monto != null ? monto.toString() : "0.00";
